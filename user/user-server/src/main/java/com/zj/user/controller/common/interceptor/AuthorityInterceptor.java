...
/**
 * Created by geely
 * 拦截器通过aop的方式对dispatchServlet进行包装
 */
...
public class AuthorityInterceptor implements HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //解析HandlerMethod  todo handler对象可以理解为一个代理对象，继承了Proxy实现了被代理对象的实现的接口
        String methodName = handlerMethod.getMethod().getName();//login
        String className = handlerMethod.getBean().getClass().getSimpleName();//userManagerController
        //解析参数,具体的参数key以及value是什么，我们打印日志
        StringBuffer requestParamBuffer = new StringBuffer();
        Map paramMap = request.getParameterMap();//获取请求的参数
        ...
        if(user == null || (user.getRole().intValue() != Const.Role.ROLE_ADMIN)){
            ...
            //上传由于富文本的控件要求，要特殊处理返回值，这里面区分是否登录以及是否有管理员权限
            if(user == null){
